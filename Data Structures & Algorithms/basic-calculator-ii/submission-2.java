class Solution {
    public int calculate(String s) {
        ...
        for (Character chr : s.toCharArray()) {
            ...
            // 1. First entry
            if (values.isEmpty() || !priorityOperators.contains(operators.get(operators.size() - 1))) {
                values.add(val);
                val = 0;
            ...
            if (priorityOperators.contains(operators.get(operators.size() - 1))) {
                // Get the last operator
                // Get the last value
                Integer first = values.remove(values.size() - 1);
        ...
        if (operators.size() > 0 && priorityOperators.contains(operators.get(operators.size() - 1))) {
            Integer first = values.remove(values.size() - 1);
            Character operator = operators.remove(operators.size() - 1);
            if (operator.equals('/')) {
                values.add(first / val);
